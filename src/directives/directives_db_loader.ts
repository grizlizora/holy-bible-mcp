...
export interface DirectiveRepositories {
  warmthRepo: WarmthRepository;
  tierRepo: TierRepository;
  modeRepo: ModeRepository;
  theologyRepo: TheologyRepository;
  metricsMap: Map<string, MetricsSchema>;
  modulesMap: Map<string, string>;
}

const KNOWN_SYNONYMS_FALLBACK: Record<string, { lemma: string; translit: string }> = {
...
export function hydrateDirectivesFromDb(
  ...
  for (const r of payload.tierRows) {
    const item: ModelTierDirective = {
      tierId: (r.level_id || r.tier_id || r.tier_key) as ModelTierKey,
      ...
      maxAllowedMode: (r.max_allowed_mode || 'medium') as ModeKey,
  ...
  for (const r of payload.modeRows) {
    ...
    if (r.accuracy_matrix_json) {
      try { accMatrix = JSON.parse(r.accuracy_matrix_json); } catch {}
    }
    const item: ModeDirective = {
      modeKey: (r.mode_id || r.mode_key) as ModeKey,
  ...
  for (const r of payload.warmthRows) {
    const item: WarmthDirective = {
      levelId: (r.level_id || r.level_key) as WarmthLevelKey,
  ...
  for (const r of payload.metricsRows) {
    ...
    if (item.languageCode === 'uk') targets.metricsMap.set('ukr', item);
    if (item.languageCode === 'en') targets.metricsMap.set('eng', item);
  }
  ...
  // 8. Prophecies
  targets.theologyRepo.propheciesList = payload.propRows.map((r: any) => {
  ...
  if (payload.commentaryRows && payload.commentaryRows.length > 0) {
    targets.theologyRepo.commentariesList = payload.commentaryRows.map((r: any) => ({
      id: r.id,
      book: r.book,
      chapter: r.chapter,
      verse: r.verse,
  ...
  if (payload.semanticRows && payload.semanticRows.length > 0) {
    targets.theologyRepo.semanticConceptsList = payload.semanticRows.map((r: any) => ({
      id: r.id,
