...
export class DirectiveStore {
  ...
  public async loadDirectives(): Promise<void> {
    ...
    try {
      ...
      try {
        const osisRows = (await this.query(`SELECT * FROM osis_book_dictionary`)) as any[];
        const aliasRows = (await this.query(`SELECT * FROM osis_aliases`)) as any[];
        if (osisRows && osisRows.length > 0) {
          ...
          for (const r of osisRows) {
            ...
            if (!OSIS_BOOK_NAMES['ukr']) OSIS_BOOK_NAMES['ukr'] = {};
            if (!OSIS_BOOK_NAMES['eng']) OSIS_BOOK_NAMES['eng'] = {};
            if (!OSIS_BOOK_NAMES['rus']) OSIS_BOOK_NAMES['rus'] = {};
            if (r.name_ukr) OSIS_BOOK_NAMES['ukr'][osis] = r.name_ukr;
            if (r.name_eng) OSIS_BOOK_NAMES['eng'][osis] = r.name_eng;
            if (r.name_rus) OSIS_BOOK_NAMES['rus'][osis] = r.name_rus;
          }
