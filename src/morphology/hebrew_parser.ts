...
const HEBREW_PARSE_CACHE = new Map<string, MorphologyBreakdown>();
const MAX_HEBREW_CACHE = 5000;

/**
 * 🔍 Parses Hebrew & Aramaic WLC morphological codes (e.g. 'V-q-3ms', 'A-V-q-3ms', 'HR/Ncfsa', 'AC/AR/Ncmse')
 */
export function parseHebrewMorphCode(code: string, isAramaicOverride?: boolean): MorphologyBreakdown {
  const raw = code.trim();
  ...
  for (const pfx of segments) {
    if (pfx === 'HC' || pfx === 'C') prefixes.push('Hebrew Conjunction (וְ)');
    else if (pfx === 'AC') prefixes.push('Aramaic Conjunction (וַ / וּ)');
    else if (pfx === 'HR' || pfx === 'R') prefixes.push('Hebrew Preposition (בְּ, לְ, כְּ, מִ)');
    else if (pfx === 'AR') prefixes.push('Aramaic Preposition (בְּ, לְ, מִן, עַל)');
    else if (pfx === 'HT' || pfx === 'T') prefixes.push('Hebrew Article (הַ)');
    else if (pfx === 'AT') prefixes.push('Aramaic Determiner / Postpositive (א-)');
    else if (pfx === 'Ad' || pfx === 'd') prefixes.push('Aramaic Relative (דִּי)');
    else if (pfx === 'Hd') prefixes.push('Hebrew Interrogative (הֲ)');
    else if (pfx === 'Hs' || pfx === 's') prefixes.push('Hebrew Relative Prefix (שֶׁ-)');
    else if (pfx === 'Hk' || pfx === 'k') prefixes.push('Hebrew Comparative Prefix (כְּ-)');
    else if (pfx === 'Ak') prefixes.push('Aramaic Comparative Prefix (כְּ-)');
  }
  ...
  if (stemPart.startsWith('V')) {
    ...
    if (HEBREW_PARSE_CACHE.size >= MAX_HEBREW_CACHE) {
      ...
      if (oldest) HEBREW_PARSE_CACHE.delete(oldest);
    }
...
export function parseAramaicMorphCode(code: string): MorphologyBreakdown {
