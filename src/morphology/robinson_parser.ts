...
function saveToGreekCache(key: string, res: MorphologyBreakdown): MorphologyBreakdown {
...
export function parseGreekMorphCode(code: string): MorphologyBreakdown {
  ...
  if (raw === 'N-PRI') return saveToGreekCache(raw, { code: raw, pos: 'Proper Noun', description: 'Proper Noun (Indeclinable)' });
  if (raw === 'N-LI') return saveToGreekCache(raw, { code: raw, pos: 'Letter', description: 'Greek Letter (Indeclinable)' });
  if (raw === 'N-OI' || raw === 'A-NUI') return saveToGreekCache(raw, { code: raw, pos: 'Numeral', description: 'Numeral (Indeclinable)' });
  if (raw === 'HEB') return saveToGreekCache(raw, { code: raw, pos: 'Hebrew Word', description: 'Hebrew Loanword in Greek' });
  if (raw === 'ARAM') return saveToGreekCache(raw, { code: raw, pos: 'Aramaic Word', description: 'Aramaic Word in Greek' });
