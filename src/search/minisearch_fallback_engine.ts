...
export interface IndexedVerseDoc {
  id: string;
  book: string;
  chapter: number;
  verse: number;
...
export class MiniSearchFallbackEngine {
  private static instance: MiniSearchFallbackEngine | null = null;
  private miniSearch: MiniSearch<IndexedVerseDoc>;
  private isIndexed = false;

  constructor() {
    this.miniSearch = new MiniSearch<IndexedVerseDoc>({
      fields: ["text", "book"],
  ...
  public static getInstance(): MiniSearchFallbackEngine {
    if (!this.instance) {
  ...
  public prewarmCoreDoctrinalVerses(): void {
    const coreVerses: IndexedVerseDoc[] = [
  ...
  public addDocuments(docs: IndexedVerseDoc[]): void {
    if (docs.length === 0) return;
    this.miniSearch.addAll(docs);
    this.isIndexed = true;
  }

  public search(query: string, limit = 30): IndexedVerseDoc[] {
