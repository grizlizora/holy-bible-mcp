...
export function estimatePromptComplexity(text: string): PromptComplexityResult {
  ...
  else if (clean.length > 120 || wordCount > 20) score += 10;
  else if (clean.length < 25 || wordCount < 4) score -= 12;
  ...
  for (const t of deepTriggers) {
    if (lower.includes(t)) deepMatches++;
  }
  for (const t of simpleTriggers) {
    if (lower.includes(t)) simpleMatches++;
  }
