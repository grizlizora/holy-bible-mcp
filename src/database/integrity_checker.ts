...
export function checkSqliteHeader(filePath: string): { valid: boolean; error?: string; pageSize?: number } {
  try {
    if (!fs.existsSync(filePath)) return { valid: false, error: "File not found on disk." };
    const stat = fs.statSync(filePath);
    if (stat.size < 512) return { valid: false, error: `File too small to be a database (${stat.size} bytes).` };
...
export async function verifyDatabaseSha256(
  filePath: string,
  expectedHash?: string
): Promise<{ valid: boolean; actualSha256: string; error?: string }> {
  try {
...
export async function verifySqliteDatabaseIntegrity(
  filePath: string,
  options?: { expectedSha256?: string; calculateSha256?: boolean }
