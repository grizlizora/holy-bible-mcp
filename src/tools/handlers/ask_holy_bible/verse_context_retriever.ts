...
export class VerseContextRetriever {
  public static async retrieveVerses(
    keywords: string[],
    detectedLang: string,
    limit: number = 6
          ...
          for (const c of concepts) {
            try {
              const row = await queryDb(
                `SELECT book, chapter, verse, text, language FROM verses WHERE UPPER(book) = ? AND chapter = ? AND verse = ? AND language = ? LIMIT 1`,
                [c.book.toUpperCase(), c.chapter, c.verse, detectedLang]
              );
              if (row && row.length > 0) {
