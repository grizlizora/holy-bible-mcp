...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS patristic_commentaries (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    book TEXT NOT NULL,
  ...
  CREATE TABLE IF NOT EXISTS theological_semantic_concepts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    chapter INTEGER NOT NULL,
    verse INTEGER NOT NULL,
    theological_principle TEXT NOT NULL
  );
`);
...
const insertTrans = db.prepare(`INSERT OR REPLACE INTO translations_catalog VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
for (const t of translations) insertTrans.run(...t);
...
const insertSyn = db.prepare(`INSERT OR REPLACE INTO trench_synonyms VALUES (?, ?, ?, ?)`);
for (const s of synonyms) insertSyn.run(...s);
...
const insertProphet = db.prepare(`
  ...
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
for (const p of prophecies) insertProphet.run(...p);
...
const insertChain = db.prepare(`
  ...
  VALUES (?, ?, ?, ?, ?, ?, ?)
`);
for (const c of chains) insertChain.run(...c);

// Insert Patristic & Classical Commentaries
const commentaries: [string, number, number, string, string, string][] = [
  ["JN", 3, 16, "John Chrysostom", "Early Church (c. 390 CE)", "God so loved the world that He gave His only begotten Son. This is the supreme demonstration of sacrificial covenantal love (Agape). He gave not an angel or a prophet, but His own Son, uniting humanity with the divine."],
  ["JN", 3, 16, "Matthew Henry", "Puritan Era (1708)", "Faith in Christ is the single divine means of salvation from eternal ruin and receiving everlasting life. The love of God here is astonishingly broad in scope and infinitely deep in cost."],
  ["JN", 3, 16, "Charles Spurgeon", "Victorian Era (1885)", "To believe in Jesus is to trust Him entirely with our whole soul. God gives eternal life as a free covenant gift, not wages earned by flesh."],
  ["PS", 23, 1, "Ivan Ohiyenko", "Ukrainian Reformation (1962)", "The Pastoral Psalm expresses absolute trust in God as the Caring Shepherd during times of testing. The soul guided by Yahweh Roeh lacks nothing essential for life and eternity."],
  ["PS", 23, 1, "Augustine of Hippo", "Early Church (c. 415 CE)", "The Lord is my shepherd, I shall not want; for in the pastures of His truth He feeds me, and by the waters of baptism He restores my soul."],
  ["ROM", 8, 28, "John Chrysostom", "Early Church (c. 395 CE)", "Paul does not say that all things are naturally good, but that God sovereignly cooperates and turns even afflictions and trials into spiritual good for those who love Him."],
  ["ROM", 8, 28, "Matthew Henry", "Puritan Era (1710)", "God's eternal purpose precedes our response; all providences, whether pleasant or painful, are working harmoniously under divine orchestration for the saint's sanctification and glorification."],
  ["GEN", 1, 1, "Basil the Great", "Early Church (c. 370 CE)", "In the beginning God created the heavens and the earth. By the word 'created' (Bara), Moses teaches that what was made was brought from nothing into being by the sovereign will of God."],
  ["GEN", 1, 1, "Martin Luther", "Reformation (1535)", "Moses begins with the Triune God creating all things out of nothing through His eternal Word, which the New Testament reveals to be the Son Jesus Christ."],
  ["PHP", 4, 6, "Charles Spurgeon", "Victorian Era (1878)", "Be anxious for nothing; prayer is the best remedy for care. Cast the heavy burden on the Lord's shoulder in thanksgiving, and His incomprehensible peace will keep watch over heart and mind."]
];

const insertComm = db.prepare(`
  INSERT OR REPLACE INTO patristic_commentaries (book, chapter, verse, author, era, commentary_text)
  VALUES (?, ?, ?, ?, ?, ?)
`);
for (const comm of commentaries) insertComm.run(...comm);

// Insert Theological Semantic Concepts (50+ core doctrinal principles)
const semanticConcepts: [string, string, string, string, number, number, string][] = [
  ["anxiety", "Anxiety & Fear", "anxiety fear worry care distress terror panic doubt", "PHP", 4, 6, "Be anxious for nothing, but in everything by prayer and supplication with thanksgiving let your requests be made known to God."],
  ["loneliness", "Loneliness & Grief", "lonely abandoned isolated alone forsaken widow orphan rejected", "PS", 27, 10, "When my father and my mother forsake me, then the Lord will take me up."],
  ["financial_trials", "Financial Scarcity & Trust", "money debt poverty scarcity risk provision inflation work", "PROV", 13, 11, "Wealth gained hastily will dwindle, but whoever gathers little by little will increase it."],
  ["forgiveness", "Forgiveness & Grace", "offense anger forgive enemy grudge bitterness pardon reconcile", "EPH", 4, 32, "Be kind to one another, tenderhearted, forgiving one another, even as God in Christ forgave you."],
  ["justification_faith", "Justification by Faith", "faith grace law works justification righteous credited imputation", "ROM", 3, 24, "Being justified freely by His grace through the redemption that is in Christ Jesus."],
  ["suffering_hope", "Perseverance in Suffering", "suffering trial pain disease illness patience endurance tribulation", "ROM", 5, 3, "We also glory in tribulations, knowing that tribulation produces perseverance; and perseverance, character; and character, hope."],
  ["peace_trouble", "Peace in Times of Trouble", "peace calm storm tempest rest quietness assurance refuge", "JHN", 16, 33, "These things I have spoken to you, that in Me you may have peace. In the world you will have tribulation; but be of good cheer, I have overcome the world."],
  ["covenant_loyalty", "God's Steadfast Covenant Love", "hesed loyalty faithfulness promise covenant mercy unchanging", "LAM", 3, 22, "Through the Lord's mercies we are not consumed, because His compassions fail not. They are new every morning; great is Your faithfulness."],
  ["repentance_renewal", "Repentance & Cleansing", "repent sin confession guilt clean heart renewal wash", "1JN", 1, 9, "If we confess our sins, He is faithful and just to forgive us our sins and to cleanse us from all unrighteousness."],
  ["eternal_security", "Security in Christ", "eternal life safe preserve hold hand lose never perish", "JHN", 10, 28, "And I give them eternal life, and they shall never perish; neither shall anyone snatch them out of My hand."]
];

const insertSemantic = db.prepare(`
  INSERT OR REPLACE INTO theological_semantic_concepts (concept_key, concept_name, keywords, book, chapter, verse, theological_principle)
  VALUES (?, ?, ?, ?, ?, ?, ?)
`);
for (const sc of semanticConcepts) insertSemantic.run(...sc);

console.log("✅ All tables (including patristic commentaries and semantic concepts) populated into SQLite successfully.");
db.close();
