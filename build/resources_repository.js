        ...
        switch (parsed.type) {
           ...
           WHERE UPPER(book) = ? AND chapter = ? AND verse = ? LIMIT 20`, [osisCode, chapter, verse]);
            ...
            case "interlinear": {
                ...
                const osisCode = normalizeOsisBook(book);
                const verseRow = await queryDb(`SELECT text, original_data FROM verses 
           WHERE UPPER(book) = ? AND chapter = ? AND verse = ? LIMIT 1`, [osisCode, chapter, verse]);
...
function parseBibleResourceUri(uri) {
...
function normalizeOsisBook(input = "") {
