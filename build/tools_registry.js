...
export function resolveLanguageCode(inputLang, sampleText) {
    ...
    if (/[äöüß]/i.test(str))
        return 'deu';
    if (/[ñ¿¡áíóú]/i.test(str))
        return 'spa';
    if (/[ãõ]/i.test(str))
        return 'por';
    if (/[èêëàâùûçœæ]/i.test(str))
        return 'fra';
    if (/[é]/i.test(str) && !/[¿¡ñ]/.test(str))
        return 'fra';
    if (/[àèéìòù]/i.test(str))
        return 'ita';
    if (/[șțăâî]/i.test(str))
        return 'ron';
    if (/[čďěňřšťůž]/i.test(str))
        return 'ces';
    if (/[áéíóöőúüű]/i.test(str))
        return 'hun';
    if (/[ğşıüöç]/i.test(str))
        return 'tur';
    return 'eng';
}
/**
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            if (name === "ask_holy_bible" || name === "build_biblical_context") {
               ...
               LIMIT 6`, [matchQuery, detectedLang]);
                                ...
                                settings: [
                                    {
                                        ...
                                        options: (store.getAllWarmthRanges().length > 0 ? store.getAllWarmthRanges() : []).map((w) => ({
                                            value: w.minScore,
