    ...
    async getRankedCrossReferences(book, chapter, verse, category = 'all', maxResults = 5, lang = 'ukr') {
        ...
        for (const neighbor of graphNeighbors) {
            ...
            try {
                ...
                if (parts.length >= 3) {
                    const rows = await queryDb(`SELECT text FROM verses WHERE UPPER(book) = ? AND chapter = ? AND verse = ? LIMIT 1`, [parts[0], Number(parts[1]), Number(parts[2])]);
                    if (rows[0]?.text) {
                        text = rows[0].text;
                    }
                }
        ...
        if (candidates.length < maxResults) {
         ...
         LIMIT 20`, [osisBook, chapter, verse, osisBook, chapter, verse]);
            for (const r of rows) {
                if (candidates.length >= maxResults)
                    break;
