...
import { ThematicChainTracer } from "./graph/thematic_chain_tracer.js";
import { ProphecyFulfillmentMatcher } from "./graph/prophecy_fulfillment_matcher.js";
...
export class ScriptureGraphEngine {
    ...
    async getRankedCrossReferences(book, chapter, verse, category = 'all', maxResults = 5, lang = 'ukr') {
        ...
        const neighborTexts = new Map();
        await Promise.all(graphNeighbors.map(async (neighbor) => {
            const parts = neighbor.targetOsis.split(".");
            if (parts.length >= 3) {
                const key = `${parts[0]}.${parts[1]}.${parts[2]}`;
                try {
                    const rows = await queryDb(`SELECT text FROM verses WHERE book = ? AND chapter = ? AND verse = ? LIMIT 1`, [parts[0], Number(parts[1]), Number(parts[2])]);
                    if (rows[0]?.text) {
        ...
        for (const neighbor of graphNeighbors) {
            if (seenRefs.has(neighbor.targetOsis))
                continue;
        ...
        if (candidates.length < maxResults) {
            try {
                ...
                for (const r of concepts) {
                    ...
                    try {
                        const targetVerseRows = await queryDb(`SELECT text FROM verses WHERE book = ? AND chapter = ? AND verse = ? LIMIT 1`, [r.book, r.chapter, r.verse]);
