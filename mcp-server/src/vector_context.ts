...
export interface ChunkOptions {
  targetChunkSize?: number; // characters (~500 tokens)
  overlapSize?: number;     // characters (~100 tokens)
}
...
export async function extractVectorContext(
  ...
  const scoredChunks = chunks.map(c => {
    ...
    for (const term of queryTerms) {
      if (cLower.includes(term)) score += 2;
    }
    // Give structural weight to intro, section headers, and concluding sections
    if (c.chunkIndex === 0 || c.chunkIndex === chunks.length - 1) score += 1;
    if (c.headingContext) score += 1.5;
  ...
  const selectedItems: typeof scoredChunks = [];
