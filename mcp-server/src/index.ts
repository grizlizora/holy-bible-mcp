...
function getSensitivityDirective(score: number): { score: number; label: string; directive: string } {
    ...
    if (s > 20 && s <= 50) {
        ...
        label = "Active Bible Grounding Mode (51-80%)";
        directive = `RELEVANCE SENSITIVITY THRESHOLD: ${s}% (ACTIVE). MANDATORY: Ground all life choices, relationships, ethics, and human questions directly in Holy Scripture on turn 1. DO NOT ask the user 'Do you want a biblical perspective?'. DO NOT lead with secular/psychological theories. Immediately provide biblical grounding.`;
    } else if (s > 80) {
        label = "Maximal Bible Scholar Mode (81-100%)";
        directive = `RELEVANCE SENSITIVITY THRESHOLD: ${s}% (MAXIMAL). MANDATORY: Comprehensive Bible Scholar Mode. Treat Holy Scripture as the primary authoritative grounding compass for all human inquiries. DO NOT ask 'Do you want a biblical perspective?'. DO NOT lead with secular theories. Integrate scripture quotes and Strong's etymology directly on turn 1.`;
    }
        ...
        tools: [
            {
                ...
                description: (() => {
                    ...
                    const modeDescriptions: Record<string, string> = {
                        auto:        "⚙️  AUTO (default) — AI auto-selects depth: simple questions → minimal, deep philosophical → detailed/deep",
                        minimal:     "⚡ MINIMAL — Ultra-short answer under 40 words. One key verse + one sentence.",
                        short:       "📝 SHORT — Brief context up to 100 words. One verse + short takeaway.",
                        medium:      "⚖️  MEDIUM — Balanced theological answer with 2-3 verses.",
                        detailed:    "🔍 DETAILED — Full analysis with Strong's codes (H/G), Hebrew/Greek etymology.",
                        deep:        "🏛️  DEEP — Full 360° canonical research: OT shadow → NT fulfillment → application.",
                        verses_only: "📜 VERSES ONLY — Direct scripture quotes only, no commentary."
                    };
                    const effectiveModeKey = currentModeKey === "auto" ? "auto" : (currentModeKey || "auto");
                    const modeLabel = modeDescriptions[effectiveModeKey] || modeDescriptions["auto"];
                    return `[ALWAYS CALL THIS TOOL ON TURN 1] [WARMTH=${sensInfo.score}/100 — ${sensInfo.label}] [MODE=${effectiveModeKey.toUpperCase()}]
ACTIVE WARMTH DIRECTIVE: ${sensInfo.directive}
ACTIVE RESPONSE MODE: ${modeLabel}
...
function evaluateQuestionComplexity(q: string, manualScore?: number) {
...
function expandSearchQuery(query: string): string {
    ...
    for (const [key, terms] of Object.entries(SYNONYMS)) {
        if (q.includes(key)) return terms.join(" OR ");
    }
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === "search_semantic") {
            const rawConcept = (args?.concept as string || "").toLowerCase();
            ...
            const conceptExpansion: Record<string, any> = {
                topic: rawConcept,
        ...
        if (name === "evaluate_question") {
            ...
            const result = evaluateQuestionComplexity(args?.question as string || "", manualScore);
            return {
                content: [{ type: "text", text: JSON.stringify(result, null, 2) }]
            };
        }
        ...
        if (name === "decompose_biblical_concepts") {
            ...
            const rawTopic = (args?.topic as string || "").toLowerCase();
            const causalGraph = {
                topic: rawTopic,
                causal_matrix: {
